...
local function tgl() if sg:FindFirstChild("Frame") and keyF.Parent then return end f.Visible, min.Visible = not f.Visible, f.Visible end
...
local function toggleBackwards()
    ...
    if hrp then if bkwd then hrp.CFrame = hrp.CFrame * CFrame.new(0, 1.5, 0) * CFrame.Angles(0, 0, math.rad(180)) hrp.Anchored = true else hrp.Anchored = false hrp.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(0, math.rad(hrp.Orientation.Y), 0) end end
end
...
local bFps = btn("Boost FPS: OFF", UDim2.new(0.525,0,0.65,0), function() fps = not fps; if fps then for _, v in pairs(game:GetDescendants()) do if v:IsA("BasePart") then v.Material = Enum.Material.Plastic; v.Reflectance = 0 elseif v:IsA("Decal") or v:IsA("Texture") then v.Transparency = 1 end end end end)
...
run.RenderStepped:Connect(function()
    ...
    bAmSGG.Text="AutoMSGG: "..(amsgg and "ON" or "OFF")
    if drag then local pct = math.clamp((UIS:GetMouseLocation().X - sBg.AbsolutePosition.X)/sBg.AbsoluteSize.X, 0, 1) sBar.Size, smooth = UDim2.new(pct,0,1,0), 0.01 + (pct * 0.9); sL.Text = string.format("Smooth: %.2f", smooth) end
    if d_fov then local pct = math.clamp((UIS:GetMouseLocation().X - fBg.AbsolutePosition.X)/fBg.AbsoluteSize.X, 0, 1) fBar.Size, fov_r = UDim2.new(pct,0,1,0), math.floor(pct * 500); fL.Text = "FOV Size: "..fov_r end
    for plr, v in pairs(ESP_Cache) do local char = plr.Character if char and char:FindFirstChild("HumanoidRootPart") and char.Humanoid.Health > 0 then local pos, vis = cam:WorldToViewportPoint(char.HumanoidRootPart.Position) if vis then local h = 2000 / pos.Z; local w = h / 1.5; v.box.Visible, v.box.Size, v.box.Position = esp, Vector2.new(w, h), Vector2.new(pos.X - w/2, pos.Y - h/2) v.line.Visible, v.line.From, v.line.To = trc, Vector2.new(cam.ViewportSize.X/2, cam.ViewportSize.Y), Vector2.new(pos.X, pos.Y) v.name.Visible, v.name.Text, v.name.Position = nm, plr.Name, Vector2.new(pos.X, pos.Y - h/2 - 20) else v.box.Visible, v.line.Visible, v.name.Visible = false, false, false end else v.box.Visible, v.line.Visible, v.name.Visible = false, false, false end end
    if aim then local target, close = nil, (fov and fov_r or 1000) for _, v in pairs(game.Players:GetPlayers()) do if v ~= p and v.Character and v.Character:FindFirstChild("Head") and v.Character.Humanoid.Health > 0 then local pos, vis = cam:WorldToViewportPoint(v.Character.Head.Position) local dist = (Vector2.new(pos.X, pos.Y) - center).Magnitude if vis and dist < close then target = v.Character.Head; close = dist end end end if target then cam.CFrame = cam.CFrame:Lerp(CFrame.lookAt(cam.CFrame.Position, target.Position), smooth) end end
end)

-- [ LOADER ]
local ldF = Instance.new("Frame", sg); ldF.Size, ldF.Position = UDim2.new(0,250,0,80), UDim2.new(0.5,-125,0.5,-40); ldF.BackgroundColor3 = Color3.new(0,0,0); Instance.new("UICorner", ldF); Instance.new("UIStroke", ldF).Color = Color3.new(1,0,0)
...
-- [ FINAL INIT ]
UIS.InputBegan:Connect(function(i, g) if not g then if i.KeyCode == Enum.KeyCode.P then tgl() end if i.KeyCode == Enum.KeyCode.J then toggleBackwards() end if i.KeyCode == Enum.KeyCode.K and dw then local m = p:GetMouse(); if m.Target then m.Target:Destroy() end end end end)
